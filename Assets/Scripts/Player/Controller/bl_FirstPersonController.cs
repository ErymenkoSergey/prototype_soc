...
public class bl_FirstPersonController : bl_MonoBehaviour
{
    ...
    private void OnCrouchChanged()
    {
        if (Crounching)
        {
            ...
            Vector3 localVelocity = m_Transform.InverseTransformDirection(Velocity);
            if (VelocityMagnitude > WalkSpeed && localVelocity.z > 0.1f)
            {
                DoSlide();
            }
        }
        ...
        if (State != PlayerState.Climbing && State != PlayerState.Sliding)
        {
            if (m_Input.sqrMagnitude > 0)
            {
                ...
                else
                {
                    ...
                    else if (VerticalInput <= 1 && VelocityMagnitude > 0.1f && State != PlayerState.Crouching)
                    {
                        State = PlayerState.Walking;
                    }
                    else if (State != PlayerState.Crouching && VelocityMagnitude > 0.1f)
                    {
                        State = PlayerState.Walking;
                    }
                    else if (State != PlayerState.Jumping && State != PlayerState.Crouching)
