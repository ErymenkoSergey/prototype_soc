...
public class bl_UIReferences : bl_PhotonHelper, IInRoomCallbacks
{
    ...
    {
        playerScoreboards.SetActive(active, (!bl_GameManager.Instance.FirstSpawnDone && bl_GameManager.Joined));
...
#if LOCALIZATION
            RespawnCountText.text = string.Format(LocaleStrings[2], remaing);
#else
            RespawnCountText.text = string.Format(bl_GameTexts.RespawnIn, remaing);
#endif
            yield return null;
        }
    }
    ...
    {
        return !inTeam && (!bl_RoomSettings.Instance.AutoTeamSelection || bl_GameData.Instance.lobbyJoinMethod == LobbyJoinMethod.DirectToMap);
    }
